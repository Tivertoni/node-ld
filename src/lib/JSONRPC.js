    ...
    req.on("end", () => {
      try {
        ...
        if (data instanceof Array) {
          async.map(data, self.doRPC.bind(self), (err, resp) =>
            res.json(err || resp.filter((v) => v))
          );
        } else self.doRPC(data, (err, resp) => res.json(err || resp));
      } catch (ex) {
        console.error("JSONRPC ERROR", ex);
        if (ex instanceof SyntaxError) res.json(self.PARSE_ERROR);
        else res.json(self.INTERNAL_ERROR, ex);
      }
    });
  }
  doRPC(data, cb) {
    ...
    self.base[data.method](...data.params, (err, resp) => {
      var ret = {};
      if (err) ret = self.error(-32000, ret.err.toString());
      else ret = self.success(resp.payload.toJSON().data);
      if (data.id) ret.id = data.id;
      else ret = null;
      cb(null, ret);
    });
  }
