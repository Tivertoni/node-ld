...
export default class ToyPad extends EventEmitter {
  constructor(opts) {
    ...
    if (!opts.transport && opts.transport !== false) {
      if (process.browser) opts.transport = new transports.ChromeHIDTransport();
      else if (process.platform == "linux")
        opts.transport = new transports.LibUSBTransport();
      else opts.transport = new transports.HIDTransport();
    }
  ...
  _write(data) {
    ...
    else throw new Error("Transport not set");
  }
  ...
  setTransport(transport) {
    ...
    this.transport.on("data", (data) => {
      this.emit("raw", data);
      if (data[0] == 0x55) this.emit("response", new Response(data));
      if (data[0] == 0x56) this.emit("event", new Event(data));
    });
